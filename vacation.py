...
def vacation_thread(sender, recipient):
    ...
    try:
        ...
        query = "SELECT 'yes' FROM outofoffice_log WHERE sender = " + "'" + sender + "'" + " AND recipient = " + \
                "'" + recipient + "'" + " AND created >= CURDATE()"
        ...
        if sent == 'na':
            ...
            for row in results:
                ...
                try:
                    ...
                    syslog.syslog(syslog.LOG_INFO, "info: sent to=<" + str(sender) + "> local-recipient=<" + str(
                        recipient) + "> reply-to=<" + str(ooofrom) + "> relay=[" + RELAY_HOST + "]")
                except Exception, e:
            ...
            if VERBOSE == 1:
                syslog.syslog(syslog.LOG_INFO,
                              "info: message already sent to=<" + str(sender) + "> local-recipient=<" + str(
                                  recipient) + ">")
    except:
